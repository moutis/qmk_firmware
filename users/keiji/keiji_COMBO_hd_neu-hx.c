...
#define EnglishComboHeldE "Thank you very much,"
...
const uint16_t PROGMEM F_CAPS_combo[] = {KC_L, KC_D, KC_U, KC_O,  COMBO_END}; // CAPS LOCK (on until hit again)
...
const uint16_t PROGMEM Hat_combo[] = {LT(4, KC_BSPC), LT(6, KC_R), COMBO_END}; // MENU
...
const uint16_t PROGMEM Hgrv_combo[] = {RGUI_T(KC_E), RCTL_T(KC_I), COMBO_END}; // ` grave (dead key)
const uint16_t PROGMEM Hmacr_combo[] = {RSFT_T(KC_A), RALT_T(KC_C), COMBO_END}; // - macron (dead key)
...
const uint16_t PROGMEM kya_combo[] = {LT(L_NUMPAD, KC_K), RSFT_T(KC_A), COMBO_END};  // きゃ
const uint16_t PROGMEM kyu_combo[] = {LT(L_NUMPAD, KC_K), KC_U, COMBO_END};  //　きゅ
const uint16_t PROGMEM kyo_combo[] = {LT(L_NUMPAD, KC_K), KC_O, COMBO_END};  //　きょ
...
const uint16_t PROGMEM hya_combo[] = {LT(L_NAV,KC_H), RSFT_T(KC_A), COMBO_END};  // びゃ
// shares a diacritic combo, so procecessed differently there (for ˚=å)
const uint16_t PROGMEM hyu_combo[] = {LT(L_NAV,KC_H), KC_U, COMBO_END};  //　びゅ
const uint16_t PROGMEM hyo_combo[] = {LT(L_NAV,KC_H), KC_O, COMBO_END};  //　びょ
...
// Japanese contract consonants
    ...
    [jp_hya] = COMBO_ACTION(hya_combo),  // ひゃ　// shared by ˚=å, so needs mode switch
...
// END the H digraphs
                ...
                case HC_TYPE_ENGLISH:
                    if (myKC_C == KC_C) { // is it English mode?
