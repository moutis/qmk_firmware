...
/*
void matrix_scan_user_process_semkey() {  // called from matrix_scan_user

        if (timer_elapsed(linger_timer) > LINGER_TIME)) {
            saved_mods = get_mods();
            clear_mods();
            switch(keycode) {  // combo_on is global, set in process_combo above

//
// These should probably all #ust be semkeys
//
                case HC_ENYE:
                    tap_code16(A(KC_N));
                    break;
