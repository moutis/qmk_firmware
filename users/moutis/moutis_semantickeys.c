...
const uint16_t SemKeys_t[SemKeys_COUNT - SK_KILL][OS_count] = {
    ...
    [SK_APPNXT - SK_KILL] = {LGUI(G(KC_TAB)),A(KC_TAB)}, // APP switcher Next (last used)
    [SK_APPPRV - SK_KILL] = {LGUI(G(S(KC_TAB))),A(S(KC_TAB))}, // APP switcher Prev (least recently used)
    [SK_WINNXT - SK_KILL] = {LGUI(C(KC_TAB)),C(KC_TAB)}, // Window/tab switcher Next
    [SK_WINPRV - SK_KILL] = {LGUI(C(S(KC_TAB))),C(S(KC_TAB))}, // Window/tab switcher Prev
