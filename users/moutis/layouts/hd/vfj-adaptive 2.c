...
//  Z   │  F   P   L   D   J  │ [ copy   pste ] │  -+   U   O   Y   B │ Q
//      ╰───────────╮ bsp  R  │ &             | │ spc  ret ╭──────────╯
...
*/
        ...
        case KC_G:
            switch (prior_keycode) {
                case KC_V: // roll VG => VL (avoid scissor, 2.93x more common)
                case KC_K: // eliminate index-middle stretch with alt fingering (3.1x more common)
        ...
        case KC_V: // remedy inner column split by shifting fingering
            switch (prior_keycode) {
                case KC_G: // 99.7% of GT are followed by H
                    ...
                    return_state = false; // done.
                    break;
                case KC_M: // Eliminate ML SFB (ML is 15x more common than MV)
                case KC_W: // Eliminate WL scissor (WL is 1101x more common than WV)
                    tap_code(KC_L); //
                    return_state = false; // done.
                    break;
                case KC_H: // 
                case KC_X: // Eliminate LV scissor/stretch (LV is 17x more common than XV)
                    tap_code(KC_BSPC);
                    send_string("lv");
                    return_state = false; // done.
           }
