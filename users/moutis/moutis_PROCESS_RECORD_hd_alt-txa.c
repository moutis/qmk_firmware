    ...
    if (!leading) {
        ...
        switch (record_keycode) {
            ...
            case KC_V: // IF KC_M precedes within ADAPTIVE_TERM, then send B to avoid row jump.
                ...
                break;
            case KC_P: // IF KC_B precedes, then send L bc hand moved out of home block.
            case KC_D: // IF KC_B precedes, then send L bc hand moved out of home block.
                ...
                break;
            case KC_B: // IF KC_C precedes, then send KC_K bc hand moved out of home block.
                if (record->event.pressed) {
                        if ((prior_keycode == KC_C) && (timer_elapsed(prior_keydown) < ADAPTIVE_TERM)) {
                            tap_code(KC_K); // quickly typing "cb" yields "ck"
                            return_state = false; // don't do more with this record.
                        }
                }
                break;
            case KC_C: // IF KC_B  precedes within ADAPTIVE_TERM, then send L bc hand moved out of home block.
                if (record->event.pressed) {
                        if ((prior_keycode == KC_B) && (timer_elapsed(prior_keydown) < ADAPTIVE_TERM)) {
                            tap_code(KC_L); // quickly typing "bc" yields "bl"
                            return_state = false; // don't do more with this record.
                    }
                }
                break;
